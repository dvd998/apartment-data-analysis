...
for i in range(1, 31):
    ...
    for link in hrefs:
        ...
        sq_meter = sq.find_next('br').next_sibling.text.strip()
        sq_meters.append(sq_meter)
        print(sq_meter)
        room = soup.find('span', text = "Sobe:")
        room_num = room.find_next('br').next_sibling.text.strip()
        room_numbers.append(room_num)
        price = soup.find(class_ = "stickyBox__price").text.split('EUR')[0].strip()
        prices.append(price)
